...
class Labyrinth:
    ...
    def __init__(self, taille, debut=None, fin=None):
        ...
        self.__distance_entre_debut_et_fin_not_used = math.ceil(
            taille / 2
        )  # arrondi par exèse de taille/2
    ...
    def remplir(self, nb_murs):
        ...
        def fixer_case_debut_fin():  # On vient ici car debut et fin = None. Il faut les fixer
            ...
            if num_case_debut not in range(N * N) and num_case_fin not in range(N * N):
                ...
                quelle_liste_debut = random.randint(
                    0, 3
                )  # On a 4 listes de contours 0..3
                # On sait que chaucun des 4 listes est de taille N : len(Les_4_listes[quelle_liste])=N
                num_case_debut = Les_4_listes[quelle_liste_debut][
                ...
                while True:
                    while quelle_liste_debut == quelle_liste_fin:
                        quelle_liste_fin = random.randint(0, 3)
                    num_case_fin = Les_4_listes[quelle_liste_fin][
                        random.randint(0, N - 1)
                    ]
    ...
    def is_free(self, L, C):
        return (
            ...
            and self.__matrice[L][C] != 4
        )


if __name__ == "__main__":
    taille = 12
    lab = Labyrinth(taille)  # sans donner debut/fin. seront générés aléatoirement
    lab.remplir(taille * taille // 2)
    print(lab)
