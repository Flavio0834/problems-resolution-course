...
def combien_de_Coul_sur_une_diagonale(
    echiquier, case, No_diag=1, couleur="B"
    ...
    temp_lst = (
        [1 for (l, c) in Liste if l == c]
        if No_diag == 1
        else [1 for (l, c) in Liste if l + c == 4]
    )
    return sum(temp_lst)
...
def valeur_d_une_case(echiquier, couleur, case):
    ...
    Adversaire = la_couleur_opposee_a(
        couleur
    )  # la couleur de l'adversaire de "Couleur"
    ...
    ND1_1 = combien_de_Coul_sur_une_diagonale(
        echiquier, case, 1, couleur
    )  # 0 si on n'est pas sur la première diagonale
    ND1_2 = combien_de_Coul_sur_une_diagonale(
        echiquier, case, 1, Adversaire
    )  # 0 si on n'est pas sur la première diagonale
    # print(f"Pour la case {case}, ND1_1, ND1_2 ", ND1_1, ND1_2)
    Val_diagonale_1 = ND1_1 - ND1_2

    ND2_1 = combien_de_Coul_sur_une_diagonale(
        echiquier, case, 2, couleur
    )  # 0 si on n'est pas sur la première diagonale
    ND2_2 = combien_de_Coul_sur_une_diagonale(
        echiquier, case, 2, Adversaire
    )  # 0 si on n'est pas sur la première diagonale
...
if __name__ == "__main__":
   ...
   """
    ...
    dico_valeurs_pour_couleur = {
        case: valeur_d_une_case(echiquier, couleur, case) for case in LV
    }
    print(
        f"Pour la couleur {couleur}, on a trouvé la liste des valeurs : \n{dico_valeurs_pour_couleur}"
    )

    liste_cles_du_dico_trie = sorted(
        dico_valeurs_pour_couleur, key=dico_valeurs_pour_couleur.get, reverse=True
    )
    print(
